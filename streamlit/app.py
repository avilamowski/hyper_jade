...
def init_session_state():
    ...
    if "sync_render_version" not in st.session_state:
        st.session_state.sync_render_version = (
            0  # Increment to force expander re-render
        )
...
def render_home_page():
    ...
    for i, submission in enumerate(submissions):
        with cols[i % 4]:
            if st.button(
                f"📝 {submission}", key=f"sub_{submission}", use_container_width=True
            ):
...
def _render_corrections_content(run_data: dict, run_key: str, sync_mode: bool):
    ...
    if corrections:
        ...
        if corr_list:
            ...
            with col1:
                if st.button("📂 Open All", key=f"open_corr_{run_key}"):
                    if sync_mode:
                        ...
                        for req_key in req_keys:
                            st.session_state.corr_expander_states[run_key][req_key] = (
                                True
                            )
                    st.rerun()
            with col2:
            ...
            if sync_mode:
                st.caption(
                    "💡 Use the toggle button inside each expander for reliable sync across runs."
                )
            ...
            for i, corr in enumerate(corr_list):
                ...
                # Add indicator if has locations
                has_locations = (
                    corr.get("locations") and len(corr.get("locations", [])) > 0
                )
                loc_indicator = " 📍" if has_locations else ""
                title = f"**{func_name}** ({req_type}) - {req_preview}{loc_indicator}"

                # Determine expanded state
                if sync_mode:
                    # Use shared per-item state
                    item_expanded = st.session_state.sync_correction_items.get(
                        req_key, False
                    )
                else:
                    # Use per-run state
                    item_expanded = st.session_state.corr_expander_states[run_key].get(
                        req_key, True
                    )
                ...
                with st.expander(title, expanded=item_expanded):
                    ...
                    # Display error locations if available (from pipeline)
                    locations = corr.get("locations", [])
                    if locations:
                        st.markdown("---")
                        st.markdown("**📍 Error Locations:**")
                        for loc in locations:
                            lines = loc.get("lines", "?")
                            fragment = loc.get("fragment", "")
                            st.markdown(f"**Lines {lines}:**")
                            st.code(fragment, language="python")

                        loc_explanation = corr.get("location_explanation", "")
                        if loc_explanation:
                            st.markdown(f"*{loc_explanation}*")
        else:
        ...
        if isinstance(metrics, dict):
            for i, (metric_name, metric_value) in enumerate(metrics.items()):
                if metric_name != "timings":
                    ...
                    if sync_mode:
                        metric_expanded = st.session_state.sync_metrics_expanded.get(
                            metric_name, False
                        )
                    else:
                    ...
                    with st.expander(
                        f"📈 {metric_name.upper()}", expanded=metric_expanded
                    ):
                        # In sync mode, add a toggle button to sync this specific metric
                        if sync_mode:
                            toggle_label = (
                                "🔒 Collapse" if metric_expanded else "🔓 Expand"
                            )
                            if st.button(
                                toggle_label, key=f"toggle_metric_{run_key}_{i}"
                            ):
                                st.session_state.sync_metrics_expanded[
                                    metric_name
                                ] = not metric_expanded
    ...
    if eval_results:
        ...
        if explanations:
            ...
            for i, (name, explanation) in enumerate(explanations.items()):
                ...
                if sync_mode:
                    expl_expanded = (
                        st.session_state.sync_eval_explanations_expanded.get(
                            name, False
                        )
                    )
                else:
    ...
    if prompts:
        ...
        with col2:
            if st.button("📁 Close All", key=f"close_prompt_{run_key}"):
                if sync_mode:
                    ...
                    for req_key in req_keys:
                        st.session_state.prompt_expander_states[run_key][req_key] = (
                            False
                        )
                st.rerun()
        ...
        for i, prompt in enumerate(prompts):
            ...
            if sync_mode:
                ...
                # Use per-run state
                item_expanded = st.session_state.prompt_expander_states[run_key].get(
                    req_key, False
                )
...
def render_submission_page():
                ...
                if timestamps:
                    for ts in timestamps:
                        ...
                        if run_key not in st.session_state.selected_runs:
                            ...
                            with col2:
                                if st.button(
                                    "➕", key=f"add_{run_key}", help="Add this run"
                                ):
                                    add_run(exp_type, ts)
                                    st.rerun()
                    ...
                    all_added = all(
                        f"{exp_type}/{ts}" in st.session_state.selected_runs
                        for ts in timestamps
                    )
                    if all_added:
    ...
    tab_code, tab_ref, tab_compare = st.tabs(
        ["📝 Submission Code", "✅ Reference Correction", "🔄 Compare Runs"]
    )
    ...
    with tab_compare:
        if not st.session_state.selected_runs:
            ...
            if st.session_state.sync_enabled:
                ...
                def on_tab_change():
                    st.session_state.sync_active_tab = (
                        st.session_state.sync_tab_selector
                    )

                selected_tab = st.radio(
                    "Select View",
                    options=tab_options,
                    index=tab_options.index(st.session_state.sync_active_tab)
                    if st.session_state.sync_active_tab in tab_options
                    else 0,
            ...
            for i, run_key in enumerate(st.session_state.selected_runs):
                with cols[i]:
                    ...
                    run_data = get_run_data(
                        config, exp_type, timestamp, submission_name
                    )

...
# Theory/RAG Experiment Runner: rag_co_theory
# Variables: RAG=true, Example Theory=false, Corrector Theory=true
...
echo "Running rag_co_theory experiment $ITERATIONS time(s)..."
echo "Config: RAG=true, Example Theory=false, Corrector Theory=true"
...
for i in $(seq 1 $ITERATIONS); do
    ...
    OUTPUT_DIR="outputs/evaluation/theory_rag_experiment/rag_co_theory/${TIMESTAMP}"
    ...
    uv run runners/run_supervised_individual_evaluator.py \
        ...
        --experiment-name "rag_co_theory_${TIMESTAMP}" \
        --verbose

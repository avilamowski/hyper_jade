...
# Supervised individual evaluator runner WITH LINTER AGENT ENABLED
# Based on eval_without_rag.sh but with linter agent for comparison experiments
# Use this to compare metrics with and without the linter agent
...
OUTPUT_DIR="outputs/evaluation/with_linter/${TIMESTAMP}"
SYSTEM_CONFIG="runners/config/with_linter.yaml"
...
uv run runners/run_supervised_individual_evaluator.py \
    ...
    --experiment-name "with_linter_${TIMESTAMP}" \
    --verbose

...
async def generate_prompts_traced(
    prompt_generator, requirements: List[Requirement], assignment_text: str
    ...
    if isinstance(prompt_generator, RAGPromptGeneratorAgent):
        return await prompt_generator.generate_prompts_batch(
            requirements, assignment_text
        )
    else:
...
def process_submission_traced(
    ...
    with trace(
        name=f"submission_{submission_index + 1}_processing", run_type="chain"
    ) as run_context:
        run_context.add_tags(
            [f"submission_index:{submission_index}", "correction_and_evaluation"]
        )
        ...
        with trace(name="generate_corrections", run_type="llm") as correction_context:
            correction_context.add_tags(
                ["code_correction", f"prompts_count:{len(generated_prompts)}"]
            )
            submission_corrections = code_corrector.correct_code_batch(
                generated_prompts, submission, assignment_text
            )
...
def load_reference_corrections(
    correction_paths: List[str],
) -> List[Union[str, ReferenceCorrection]]:
        ...
        if path.suffix == ".json":
            ...
            with open(corr_path, "r", encoding="utf-8") as f:
                ...
                if isinstance(data, list):
                    ...
                    logger.warning(
                        f"Unknown JSON format in {corr_path}, treating as empty correction list"
                    )
...
def save_results(
    ...
    # Build corrections data, including optional error locator fields
    corrections_list = []
    for corr in corrections:
        ...
        # Include error locator fields if present
        if corr.get("locations"):
            corr_data["locations"] = corr["locations"]
        if corr.get("location_explanation"):
            corr_data["location_explanation"] = corr["location_explanation"]
        # Include original index for trace verification
        if corr.get("_original_index") is not None:
            corr_data["_original_index"] = corr["_original_index"]
        corrections_list.append(corr_data)
...
Note: Reference corrections can be provided as either:
  ...
  - .json files (structured corrections as a list of strings)
    ...
    parser.add_argument(
        ...
        "-a",
        required=True,
        help="Path to assignment description file (.txt)",
    )
    parser.add_argument(
        "--requirements",
        "-r",
        nargs="+",
        required=True,
        help="Paths to requirement files (.json)",
    )
    parser.add_argument(
        "--submissions",
        "-s",
        nargs="+",
        required=True,
        help="Paths to student submission files (.py)",
    )
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="Paths to reference correction files (.txt or .json)",
    )
    parser.add_argument(
        "--output-dir", "-o", required=True, help="Output directory for results"
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--evaluator-config",
        default="src/config/evaluator_config.yaml",
        help="Evaluator configuration file path",
    )
    parser.add_argument(
        "--experiment-name",
        "-e",
        help="Experiment name for LangSmith tracking (optional)",
    )
    ...
    if LANGSMITH_AVAILABLE:
        if not os.environ.get("LANGSMITH_API_KEY") or not os.environ.get(
            ...
            logger.warning(
                "LangSmith available but not fully configured; traces may be incomplete"
            )
        else:
            logger.info(
                f"🔗 LangSmith tracing enabled for project: {os.environ.get('LANGSMITH_PROJECT')}"
            )
    else:
        logger.info(
            "LangSmith library not installed; running without LangSmith tracing"
        )
    ...
    logger.info(
        f"🤖 Prompt Generator Model: {prompt_config.get('model_name', 'Unknown')}"
    )
    logger.info(
        f"🤖 Code Corrector Model: {corrector_config.get('model_name', 'Unknown')}"
    )
    ...
    try:
        ...
        with trace(
            name="supervised_individual_evaluation_pipeline", run_type="chain"
        ) as pipeline_context:
            ...
            with mlflow_logger.run(
                run_name="prompt_generation", tags=mlflow_tags
            ) as prompt_run_id:
                ...
                if enable_rag:
                    generated_prompts = asyncio.run(
                        generate_prompts_traced(
                            prompt_generator, requirements, assignment_text
                        )
                    )
                else:
                    generated_prompts = asyncio.run(
                ...
                prompt_cost_info = get_run_token_usage_and_cost(
                    trace_id=pipeline_trace_id
                )
                if prompt_cost_info.get("total_tokens", 0) > 0:
                    mlflow_logger.log_metric(
                        "prompt_gen_total_tokens",
                        float(prompt_cost_info.get("total_tokens", 0)),
                    )
                    mlflow_logger.log_metric(
                        "prompt_gen_estimated_cost_usd",
                        float(prompt_cost_info.get("estimated_cost_usd", 0.0)),
                    )
                ...
                for j, prompt in enumerate(generated_prompts):
                    prompt_data = {
                        "requirement": {
                            ...
                            "type": prompt["requirement"]["type"].value
                            if hasattr(prompt["requirement"]["type"], "value")
                            else str(prompt["requirement"]["type"]),
                        },
                    ...
                    prompt_file = (
                        prompts_dir
                        / f"prompt_{j + 1:02d}_{prompt['requirement']['function']}.json"
                    )
                    with open(prompt_file, "w", encoding="utf-8") as f:
                        json.dump(prompt_data, f, indent=2, ensure_ascii=False)

                logger.info(
                    f"✓ Saved {len(generated_prompts)} prompts to {prompts_dir}"
                )

            logger.info(
                "Step 2: Processing submissions with unified LangSmith traces..."
            )
            all_evals = []
            for i, submission in enumerate(submissions):
                ...
                with mlflow_logger.run(
                    run_name=f"supervised_individual_evaluation_{i + 1}",
                    tags=submission_mlflow_tags,
                ) as run_id:
                    logger.info(f"Processing submission {i + 1}/{len(submissions)}")
                    shared.stage = "code_correction"
                    submission_corrections, evaluation_results = (
                    ...
                    mlflow_logger.log_artifacts(
                        str(submission_output_dir),
                        artifact_path=submission_output_dir.name,
                    )
                    ...
                    for aux_name, aux_value in aux_metrics.items():
                        ...
                        mlflow_logger.log_metric(
                            f"aux_{aux_name}_length", len(aux_value)
                        )

                    mlflow_logger.log_metric(
                        "num_generated_corrections", len(submission_corrections)
                    )
            ...
            total_corrections = sum(
                len(e.get("evaluation", {}).get("corrections", [])) for e in all_evals
            )
            ...
            overall_scores = [
                ...
                for e in all_evals
                if isinstance(e.get("evaluation"), dict)
                and "overall_score" in e["evaluation"]
            ]
            ...
            if cost_info.get("total_tokens", 0) > 0:
                ...
                logger.info(
                    f"Total token usage: {cost_info.get('total_tokens', 0)} tokens"
                )
                logger.info(
                    f"Total estimated cost: ${cost_info.get('estimated_cost_usd', 0.0):.6f}"
                )
